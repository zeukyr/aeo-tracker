...
def get_citation_rate_by_engine(days=None):
    filter_clause = _date_filter(days)
    query = f"""
        SELECT DATE(created_at) as day, engine,
            AVG(CASE WHEN qc_cited THEN 1 ELSE 0 END) as citation_rate
...
def get_citation_rate_by_category(days=None):
    filter_clause = _date_filter(days)
    query = f"""
...
def get_citation_rate_by_school(days=None):
    filter_clause = _date_filter(days)
    query = f"""
        SELECT q.school,
            AVG(CASE WHEN m.qc_cited THEN 1 ELSE 0 END) as citation_rate
