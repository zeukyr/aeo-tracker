...
function reachSteps(rec) {
    ...
    voteStep(router, vote),
    {
      ...
      fact: (
        <>
          ...
          <Check pass={false}>non-ownable {pctLabel(vote.non_ownable_share)}</Check> — an owned page
          can't take this slot
        </>
      ),
    },
  ];
  ...
  steps.push({
    ...
    fact: "Can't own the slot — earn it on the source engines already trust.",
    verdict: true,
  });
  return steps;
}
